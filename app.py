...
def configure_interface():
  ...
  if uploaded_file is not None:
    ...
    if blob_url:
      ...
      credit_card_info = analize_credit_card(blob_url) # Chamar a funclçai de detecção de informações de cartão de credito
...
def show_image_and_validation(blob_url, credit_card_info):
  ...
  if credit_card_info and credit_card_info["card_name"]:
    ...
    st.write(f"Banco Emissor: {credit_card_info['back_name']}")
    st.write(f"Data Validade: {credit_card_info['expiry_date']}") 
  else:
    st.markdown(f"<h1 style='color: red;'>Cartão Inválido</h1>", unsafe_allow_html=True)
    st.write(f"Este não é um número de cartão válido.")
